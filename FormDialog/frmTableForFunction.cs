...
{
    public partial class frmTableForFunction : Form
    {
        UCFunction _parent;
        public frmTableForFunction(UCFunction parent)
        {
            InitializeComponent();
            this._parent = parent;
        }

        private async void frmTableForFunction_Load(object sender, EventArgs e)
        {
        ...
        private async void btnGenerateFromTable_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in this.lvSelectedTable.Items)
            {
                ...
                sb.Append(await Helper.GenerateFunctionParamFromTable(item.SubItems[0].Text));
                sb.Append(Environment.NewLine);
                sb.Append(Environment.NewLine);
        ...
        {
            string code = Helper.GenerateFunctionParamFromListView(lvSelectedColumn);
